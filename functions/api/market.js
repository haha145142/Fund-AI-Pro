...
function parseTencent(text,symbol){
  const re=new RegExp(`v_${symbol}="([^"]*)"`);
  const m=text.match(re); if(!m) return null;
  const p=m[1].split('~');
  if(p.length<6) return null;
  return {name:p[1],price:num(p[3]),change:num(p[4]),pct:num(p[5])};
}
...
async function getIndexes(){
  ...
  return INDEXES.map(x=>{
    ...
    let validated=false;
    if(east&&t){
      const priceDiff=Math.abs(east.price-t.price);
      const pctDiff=Math.abs(east.pct-t.pct);
      validated=priceDiff<=0.05&&pctDiff<=0.03;
    }
...
async function getMarketTurnoverAndFlows(){
  ...
  const fields='f2,f3,f4,f12,f14,f48,f62,f66,f72,f78,f84';
  const url=`${EM_BASE}/api/qt/ulist.np/get?secids=${secids}&fields=${fields}&fltt=2&invt=2&_=${Date.now()}`;
  ...
  return {turnover,superLarge,large,medium,small,source:'东方财富'};
}

async function getSectorFlows(){
  const fields='f12,f14,f2,f3,f62,f184,f66,f69,f72,f75,f78,f81,f84,f87,f204,f205';
  const base=`${EM_BASE}/api/qt/clist/get?pn=1&pz=80&po=1&np=1&fltt=2&invt=2&ut=8dec03ba335b81bf4ebdf7b29ec27d15&fs=m:90+t:2+f:!50&fields=${fields}&_=${Date.now()}`;
  const j=await getJson(base); const rows=j?.data?.diff||[];
  const clean=rows.map(r=>({name:r.f14,code:r.f12,pct:num(r.f3),net:money(r.f62)})).filter(x=>x.name&&x.net!=null);
  const inflow=[...clean].sort((a,b)=>b.net-a.net).slice(0,5);
  const outflow=[...clean].sort((a,b)=>a.net-b.net).slice(0,5);
  return {inflow,outflow,source:'东方财富行业板块资金流'};
}
...
export async function onRequestGet(){
  ...
  return new Response(JSON.stringify({ok,...data}),{
    status:ok?200:206,
    headers:{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store, no-cache, must-revalidate','Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'GET,OPTIONS'}
  });
}
